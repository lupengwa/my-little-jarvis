...
const progresBars = [{"name":"Job Seeking", "val":100}, {"name":"Coding", "val":40}, {"name":"System Design", "val":0}, {"name":"Leadership", "val":20}]
...
function App() {
    ...
    const getInfo = async () => {
        try {
            ...
            //set state to pass the data to app state
        }
        catch(err) {
            console.log(err);
        }
    ...
    useEffect(() => {
        // call getInfo(); in a place that only load once
    ...
    return (
        <div className="App">
            ...
            <div>
                {
                    progresBars.map((progressBar) => {
                        return <ProgressBar key= {progressBar.name} name={progressBar.name} progress={progressBar.val}/>
                    })
                }
            </div>
